...
class TimedLock(object):
    ...
    def __enter__(self):
        ...
        if sleep_amount >= 0:
            ...
            pacvert.logger.debug('Sleeping %s (interval)', sleep_amount)
            time.sleep(sleep_amount)
        while not self.queue.empty():
            try:
                seconds = self.queue.get(False)
                pacvert.logger.debug('Sleeping %s (queued)', seconds)
                time.sleep(seconds)
            except Queue.Empty:
                continue
    ...
    def snooze(self, seconds):
        ...
        pacvert.logger.info('Adding %s to queue', seconds)
