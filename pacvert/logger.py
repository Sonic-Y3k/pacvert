...
class BlacklistFilter(logging.Filter):
    ...
    def filter(self, record):
        if not pacvert.CONFIG.LOG_BLACKLIST:
            return True
...
def initLogger(console=False, log_dir=False, verbose=False):
    """
    Setup logging for Pacvert. It uses the logger instance with the name
    'pacvert'. Three log handlers are added:
    * RotatingFileHandler: for the file pacvert.log
    ...
    * StreamHandler: for console (if console)
    ...
    be invoked multiple times, during different stages of Pacvert.
    """
    ...
    if not pacvert.DEV and pacvert.CONFIG:
        for handler in logger.handlers:
